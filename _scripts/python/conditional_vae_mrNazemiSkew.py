...
'''
h_q1 = Dense(1024, activation='relu')(inputs)
h_q = Dense(512, activation='relu')(h_q1)
h_q_y = Dense(512, activation='relu')(h_q)
h2_q_y = Dense(200 ,activation='relu')(h_q_y)
h3_q_y = Dense(100 ,activation='relu')(h2_q_y)

mu = Dense(n_z, activation='linear')(h3_q_y)
log_sigma = Dense(n_z, activation='linear')(h3_q_y)
'''
h_q = Dense(512, activation='relu')(inputs)
...
decoder_hidden1 = Dense(256, activation='relu')
...
'''
decoder_hidden = Dense(512, activation='relu')
decoder_hidden1 = Dense(1024, activation='relu')
decoder_hidden_y = Dense(512, activation='relu')
decoder_hidden2_y = Dense(200, activation='relu')
decoder_hidden3_y = Dense(100, activation='relu')
#decoder_hidden1 = Dense(1024, activation='relu')
#decoder_hidden2 = Dense(800, activation='relu')
decoder_out = Dense(784, activation='sigmoid')

h_p_y = decoder_hidden_y(z_cond)
h_p_y2 = decoder_hidden2_y(h_p_y)
h_p_y3 = decoder_hidden3_y(h_p_y2)
h_p = decoder_hidden(h_p_y3)
h_p1 = decoder_hidden1(h_p)
#h_p2 = decoder_hidden1(h_p)h_q
outputs = decoder_out(h_p1)
...
# reuse decoder layers to define decoder separately
d_in = Input(shape=(n_z+n_y,))
d_h_y = decoder_hidden_y(d_in)
d_h2_y = decoder_hidden2_y(d_h_y)
d_h3_y = decoder_hidden3_y(d_h2_y)
d_h = decoder_hidden(d_h3_y)
d_h1 = decoder_hidden1(d_h)
#d_h2 = decoder_hidden1(d_h)
...
cvae_hist = cvae.fit([X_train, y_train], XtrainDeSkew, batch_size=m, epochs=n_epoch,
							validation_data = ([X_test[1000:], y_test[1000:]], XtestDeSkew[1000:]),
...
'''
with open("conditionalSkew.txt", "a") as myfile:
    for i in range(999):#X_test.shape[0]):
        ztmp = encoder.predict([np.reshape(np.transpose(X_test[i,:]),[1,X_test.shape[1]]),np.reshape(np.transpose(y_test[i,:]),[1,y_test.shape[1]])])
        print(ztmp)
